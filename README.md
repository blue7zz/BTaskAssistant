...
- 可选择 PI 或 Codex 进行多轮需求访谈；AI 只读检查指定项目，并以结构化问题暴露范围、行为和验收缺口。
...
Plane 收集阶段使用本机 `omp` 的无工具、无规则、无扩展、无会话模式提炼候选信息；
需求访谈阶段可以选择 PI 或 Codex，并使用固定输入包和结构化输出协议。PI 仅开放读取、搜索和文件定位工具，
Codex 强制使用 `read-only` 沙箱。两者都不能批准需求或推进任务状态。

开发阶段仍采用复制已确认提示词、在外部执行并记录真实结果的方式，直到 OMP RPC 的工具审批、会话恢复、
工作目录和失败处理完成。浏览器预览不启动本机 AI CLI。
