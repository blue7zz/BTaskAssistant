...
| # | 验收场景 | 自动化覆盖 | 状态 |
|---|---|---|---|
| 1 | A 发消息后切到 B，再返回 A，消息/运行状态/审批仍在，且无 B 的事件泄漏 | TestConcurrentActivateNoCrossWrite + 事件按 tabId 分发（单实例保活） | ✅ 已实现（阶段 3） |
| 2 | A、B 都能后台运行，各自写入自己的工作树和会话 | TestTaskSessionIsolation（会话目录隔离） | ✅ 已实现 |
...
| 5 | 工作树重新绑定后不操作旧路径 | TestRxWorkspaceForTask（ready/非 ready/无绑定三态）+ WorkspaceIdentity.Generation | ✅ 已实现（阶段 2） |
...
1. 任务绑定 Git 工作树后打开 RX：界面加载、对话、工具调用、审批。
