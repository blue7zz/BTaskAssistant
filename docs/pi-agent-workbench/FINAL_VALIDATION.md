...
已完成本机桌面构建、应用进程启动与 WebKit 主页面加载检查。隔离构建进程持续存活，系统日志
记录主 frame 完成加载；筛选到的 error/fault 日志中未发现崩溃或 WebKit 加载失败。Computer
Use 可以精确识别该隔离构建为运行中，但重置会话并重新激活窗口后，可访问性树读取仍然超时。
因此原生 macOS GUI 截图/点击自动化仍未完成，桌面视觉、真实文件选择器、真实 provider 长对话
和 Windows/Linux 包属于需要人工回归项。
