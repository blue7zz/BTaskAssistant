...
//
// Host-embedding escape hatch: when Reasonix is embedded in a third-party app's
// iframe (e.g. BTaskAssistant's RX tab), Wails injects a *partial* window.go
// shell whose binding methods are absent, so `window.go.main.App` exists but
// every call fails ("Lc.Platform is not a function"). The host passes an
// explicit `?browser=1` query param to force the mock session flow instead of
// a half-broken native shell.
function isBrowserMock(): boolean {
  if (typeof window === "undefined") return true;
  return new URLSearchParams(window.location?.search ?? "").has("browser");
}

function realApp(): AppBindings | undefined {
  if (isBrowserMock()) return undefined;
...
// ── Host-embedding transport (?host=1) ────────────────────────────────────
// BTaskAssistant's RX tab embeds Reasonix in a same-origin iframe. Wails
// forbids bridge calls from non-main frames, so every bound call travels over
// postMessage: the iframe posts {source:"reasonix",type:"call",id,method,args}
// and the host main frame resolves it against the fused Go bindings, replying
// {source:"btask-reasonix",type:"result",id,value|error}. Kernel events flow
// the other way as {source:"btask-reasonix",type:"event",payload}.
const HOST_SOURCE = "reasonix";
...
const hostPending = new Map<number, HostPending>();
...
function hostCall(method: string, args: unknown[]): Promise<unknown> {
  ...
  const timer = window.setTimeout(() => {
    ...
    reject(new Error(`Reasonix 宿主桥调用超时: ${method}`));
  }, 120000);
  hostPending.set(id, {
    resolve: (value) => {
  ...
  window.parent.postMessage({ source: HOST_SOURCE, type: "call", id, method, args }, "*");
  return promise;
}

function hostSubscribe(cb: (payload: unknown) => void): () => void {
  hostEventListeners.add(cb);
  return () => {
    hostEventListeners.delete(cb);
  };
}

function installHostBridge(): void {
  if (typeof window === "undefined") return; // tsx 测试等 Node 环境无 window
  window.addEventListener("message", (event) => {
    // 只接受宿主主 frame 的应答与事件
    if (event.source !== window.parent) return;
    const data = event.data as
      | { source?: string; type?: string; id?: number; value?: unknown; error?: string; payload?: unknown }
      | undefined;
    if (!data || data.source !== HOST_REPLY_SOURCE) return;
    if (data.type === "result" && typeof data.id === "number") {
      const pending = hostPending.get(data.id);
      if (!pending) return;
      hostPending.delete(data.id);
      if (data.error) {
    ...
    if (data.type === "event") {
      hostEventListeners.forEach((listener) => listener(data.payload));
    }
  });
}
