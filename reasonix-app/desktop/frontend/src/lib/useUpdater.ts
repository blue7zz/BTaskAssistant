...
import { app, isHostMode, onUpdaterProgress } from "./bridge";
...
function useUpdaterInternal(): Updater {
  ...
  const check = useCallback(async (channel = "") => {
    // 更新检查已从本部署移除（宿主融合场景无更新机制）：
    // 无论何种模式都不调用更新 API，直接报告"已是最新"。
    // 如需恢复桌面端独立应用的更新检查，可将下方直接返回改为调用
    // app.CheckUpdate(channel) 并恢复 beginOperation/checking 状态流。
    setStatus({ kind: "upToDate", current: "" });
    return;
    void channel;
    // —— 以下为已移除的更新检查实现（保留类型引用避免无关重构）——
    // const info = await app.CheckUpdate(channel);
    const info: UpdateInfo | null = null;
    void info;
    if (true) return;
...
export function UpdaterProvider({ children }: { children: ReactNode }) {
  // 宿主嵌入模式（BTask RX 标签）：彻底禁用更新逻辑——status 恒为已是最新
  if (isHostMode()) {
    return createElement(UpdaterContext.Provider, {
      value: {
